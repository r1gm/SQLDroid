...
public class SQLDroidPreparedStatement implements PreparedStatement {
    ...
    public SQLDroidPreparedStatement(String sql, SQLDroidConnection sqldroid, String[] columnNames) {
    ...
    private String[] makeArgListQueryString() {
        ...
        for (Object o : l) {
            if (o == null) {
                ...
                if (dateFormat == null) {
                    dateFormat = new SimpleDateFormat(SQLiteDatabase.DATE_PATTERN);
                }
                strList.add(dateFormat.format(o));
            } else if (o instanceof java.sql.Time) {
                if (timeFormat == null) {
                    timeFormat = new SimpleDateFormat(SQLiteDatabase.TIME_PATTERN);
                }
                strList.add(timeFormat.format(o));
            } else if (o instanceof java.sql.Timestamp) {
                if (timeFormat == null) {
                    timeFormat = new SimpleDateFormat(SQLiteDatabase.TIMESTAMP_PATTERN);
                }
                strList.add(timeFormat.format(o));
            } else {
    ...
    public int executeUpdate() throws SQLException {
        ...
        if (autoGeneratedKeys == PreparedStatement.RETURN_GENERATED_KEYS) {
            ...
            rowIdResultSet = sqldroidConnection.getReturnedColumnResultSet(getSQL(), columnNames);
            return 1;  // This should be more efficient than sqldroidConnection.changedRowsCount()
        }
        return updateCount;
    }
     ...
     * @param inputStream the input stream from which a byte array will be read
     * and set as the value. If inputStream is null this method will throw a
     * SQLException
     * @param length a positive non-zero length values
     * @exception SQLException thrown if the length is &lt;= 0, the inputStream
     * is null, there is an IOException reading the inputStream or if "setBytes"
    ...
    public void setDate(int parameterIndex, Date theDate) throws SQLException {
        ...
        if (theDate instanceof java.sql.Date) {
            setObj(parameterIndex, theDate);
        } else {
            setObj(parameterIndex, theDate != null ? new java.sql.Date(theDate.getTime()) : null);
        }
    }
    ...
    public void setTime(int parameterIndex, Time theTime) throws SQLException {
        ensureCap(parameterIndex);
        if (theTime instanceof java.sql.Time) {
            setObj(parameterIndex, theTime);
        } else {
            setObj(parameterIndex, theTime != null ? new java.sql.Time(theTime.getTime()) : null);
        }
    }
    ...
    public void setTimestamp(int parameterIndex, Timestamp theTimestamp) throws SQLException {
        ensureCap(parameterIndex);
        if (theTimestamp instanceof java.sql.Timestamp) {
            setObj(parameterIndex, theTimestamp);
        } else {
            setObj(parameterIndex, theTimestamp != null ? new java.sql.Timestamp(theTimestamp.getTime()) : null);
        }
    }
     ...
     *
     * @see #setBinaryStream(int, InputStream, int)
     * @exception SQLException thrown if length is greater than
     * Integer.MAX_VALUE or if there is a database error.
     */
    @Override
